// Copyright Space Kiwi Studio. All Rights Reserved.

#include "UGitTaggingVersionStyle.h"
#include "UGitTaggingVersion.h"
#include "Framework/Application/SlateApplication.h"
#include "Styling/SlateStyleRegistry.h"
#include "Slate/SlateGameResources.h"
#include "Interfaces/IPluginManager.h"
#include "Styling/SlateStyleMacros.h"

#define RootToContentDir Style->RootToContentDir

TSharedPtr<FSlateStyleSet> FUGitTaggingVersionStyle::StyleInstance = nullptr;

void FUGitTaggingVersionStyle::Initialize()
{
	if (!StyleInstance.IsValid())
	{
		StyleInstance = Create();
		FSlateStyleRegistry::RegisterSlateStyle(*StyleInstance);
	}
}

void FUGitTaggingVersionStyle::Shutdown()
{
	FSlateStyleRegistry::UnRegisterSlateStyle(*StyleInstance);
	ensure(StyleInstance.IsUnique());
	StyleInstance.Reset();
}

FName FUGitTaggingVersionStyle::GetStyleSetName()
{
	static FName StyleSetName(TEXT("UGitTaggingVersionStyle"));
	return StyleSetName;
}


const FVector2D Icon16x16(16.0f, 16.0f);
const FVector2D Icon20x20(20.0f, 20.0f);

TSharedRef< FSlateStyleSet > FUGitTaggingVersionStyle::Create()
{
	TSharedRef< FSlateStyleSet > Style = MakeShareable(new FSlateStyleSet("UGitTaggingVersionStyle"));
	Style->SetContentRoot(IPluginManager::Get().FindPlugin("UGitTaggingVersion")->GetBaseDir() / TEXT("Resources"));

	Style->Set("UGitTaggingVersion.PluginAction", new IMAGE_BRUSH_SVG(TEXT("github-ico"), Icon20x20));
	return Style;
}

void FUGitTaggingVersionStyle::ReloadTextures()
{
	if (FSlateApplication::IsInitialized())
	{
		FSlateApplication::Get().GetRenderer()->ReloadTextureResources();
	}
}

const ISlateStyle& FUGitTaggingVersionStyle::Get()
{
	return *StyleInstance;
}
